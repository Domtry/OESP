...
export class SodiumCryptoProvider implements CryptoProvider {
  ...
  x25519Open(recipientPriv: Uint8Array, sealed: Uint8Array): Uint8Array {
    ...
    const pub = (sodium as any).crypto_scalarmult_base(recipientPriv);
    const opened = (sodium as any).crypto_box_seal_open(sealed, pub, recipientPriv);
    return opened;
  }
  aeadEncrypt(key: Uint8Array, plaintext: Uint8Array, aad: Uint8Array): { iv: Uint8Array; ct: Uint8Array; tag?: Uint8Array } {
    const iv = (sodium as any).randombytes_buf((sodium as any).crypto_aead_chacha20poly1305_ietf_NPUBBYTES);
    const ct = (sodium as any).crypto_aead_chacha20poly1305_ietf_encrypt(plaintext, aad, null, iv, key);
  ...
  aeadDecrypt(key: Uint8Array, iv: Uint8Array, ct: Uint8Array, aad: Uint8Array): Uint8Array {
    return (sodium as any).crypto_aead_chacha20poly1305_ietf_decrypt(null, ct, aad, iv, key);
  }
  randomBytes(n: number): Uint8Array {
    return (sodium as any).randombytes_buf(n);
  }
}
